...
/**
 * Created by gesab on 4/21/2016.
 */

public abstract class GameTicker extends Timer {
    private long totalTime, interval, delay;
    private TimerTask task;
    private long startTime = -1;
    private boolean restart = false, wasCancelled = false, wasStarted = false;

    public GameTicker(long totalTime, long interval) {
        this(totalTime, interval, 0);
    }

    public GameTicker(long totalTime, long interval, long delay) {
        super("GameTicker", true);
        this.delay = delay;
